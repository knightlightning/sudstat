...
courts = [
    ...
    ('firebird+fdb://sysdba:masterkey@10.55.65.42:49152/C:/DATA/JUSTICE/UNI_WORK2003.GDB?charset=win1251','Любинский районный суд',[0,1,2]),
    ('firebird+fdb://sysdba:m@10.55.66.42:49152/C:/DATA/JUSTICE/UNI_WORK2003.GDB?charset=win1251','Марьяновский районный суд',[0,1,2]),
    ...
    ('firebird+fdb://sysdba:m@10.55.73.42:49152/C:/DATA/JUSTICE/UNI_WORK2003.GDB?charset=win1251','Оконешниковский районный суд',[0,1,2]),
    ('firebird+fdb://sysdba:masterkey@10.55.74.42:49152/C:/DATA/JUSTICE/UNI_WORK2003.GDB?charset=win1251','Омский районный суд',[0,1,2]),
...
def getCharge(court, chargeClass):
    res = session.query(chargeClass).filter(and_(chargeClass.court_id == court_id, chargeClass.year == curr_date.year)).first()
    if res == None:
        charge = chargeClass(year=curr_date.year, court_id=court, modification=datetime.now().date())
        session.add(charge)
        session.commit()
        return charge
    return res

def getJudge(court_id, ext_id, conn, chargeType):
...
#         continue
    print_sse(SSEType.info, 'Court "{}" is being processed...'.format(court[1]))
    try:
        ...
        with engine.connect() as remote_conn:
            ...
            for proc in [ProcudureType(i).name for i in court[2]]:
                print_sse(SSEType.info, 'Procedure "{}" is being processed...'.format(proc))
                try: 
                    ...
                    charge = getCharge(court_id, chargeCls)
                    for judge_id, stat_type, stat_val in res:
                        ...
                        if data == None:
                            judge = getJudge(court_id, judge_id, remote_conn, proc)
                        ...
                        session.commit()
                    print_sse(SSEType.info, 'Procedure "{}" has been processed!'.format(proc))
                except Exception as e:
                    print_sse(SSEType.error, 'There\'s error while handling "{}" procedure: {}'.format(proc, e))
            print_sse(SSEType.info, 'Court "{}" has been processed!'.format(court[1]))
    except Exception as e:
        print_sse(SSEType.error, 'There\'s error while handling "{}" court: {}'.format(court[1], e))
